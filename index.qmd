...
The zip codes are categorial values. They are meant to be used to group items together, in this instance homes by their geographic location. They are not meant to be used to order them. For example: a home with zip code 1 is no less valuable than a home with zip code 2. This also means that this data is non-ordinal as the order of zip codes has no inherent relationship to house prices.
...
The sckit learning module itself (the one we installed) does not suport categorial vaiables. In section 1.10, the 4th bullet point under advantages states: "Able to handle both numerical and categorical data. However, the scikit-learn implementation does not support categorical variables for now." Since this is the method we are using in pyton, it does not work as well as R. R allows for categorial variables to be used in the decision tree model. There is a feature of R that is not available in Python: "Categorical variables are supported in R (rpart package)."
